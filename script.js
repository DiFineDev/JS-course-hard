...
let appData = {
    ...
    addExpensesBlock: function(){
        ...
        appData.allowNumKeys();
        appData.allowLetterKeys();
    ...
    allowNumKeys: function() {       
        inputWithNum = document.querySelectorAll('[placeholder="Сумма"]');
        for (let i = 0; i < inputWithNum.length; i++) {
            inputWithNum[i].addEventListener('input', function() {
                inputWithNum[i].value = inputWithNum[i].value.replace(/[^0-9]/,''); 
            });
        }
        
    },
    allowLetterKeys: function(){  
        let inputWithLetter = document.querySelectorAll('[placeholder="Наименование"]'); 
        for (let i = 0; i < inputWithLetter.length; i++) {
            inputWithLetter[i].addEventListener('input', function() {
                inputWithLetter[i].value = inputWithLetter[i].value.replace(/[^а-я, \t]/i,''); 
            });
        }
    }
};
